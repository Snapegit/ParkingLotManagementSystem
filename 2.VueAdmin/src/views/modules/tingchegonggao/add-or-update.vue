	...
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="发布人" prop="faburen">
					<el-input v-model="ruleForm.faburen" placeholder="发布人" clearable  :readonly="ro.faburen"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else class="input" label="发布人" prop="faburen">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-if="type!='info'"  label="公告内容" prop="gonggaoneirong">
					<editor 
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 20px 0"}' v-else-if="ruleForm.gonggaoneirong" label="公告内容" prop="gonggaoneirong">
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.gonggaoneirong = this.ruleForm.gonggaoneirong.replace(reg,'../../../springbootq6l1n/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `tingchegonggao/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.tingchegonggaoCrossAddOrUpdateFlag = false;
						 this.parent.search();
